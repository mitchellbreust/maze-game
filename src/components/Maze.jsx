...
const Character = styled.div`
    width: 40px;
    height: 40px;
    background-color: yellow; /* Green color for the frog */
    border-radius: 50% 50% 40% 40%; /* Oval shape for the body */
    position: absolute;
    border: 3px solid yellow; /* Darker green border */
    box-shadow: 0 0 10px rgba(0, 0, 0, 0.3), inset 0 0 5px rgba(255, 255, 255, 0.3);
    display: flex;
    justify-content: center;
    align-items: center;
    ...
    &::before {
        ...
        width: 20px;
        height: 10px;
        border-bottom: 3px solid black; /* Smile curve */
        border-radius: 0 0 50% 50%; /* Curved smile */
        bottom: 5px;
        left: 50%;
        transform: translateX(-50%);
    }
...
const Maze = () => {
    ...
    useEffect(() => {
        const interval = setInterval(() => {
            ...
            setMazeOffset((prevOffset) => prevOffset + 4); // Move the maze down by 4px
        }, 100); // 0.1 seconds
        ...
        <>
            <MazeCont offset={mazeOffset}>
                ...
                <Character style={{ top: `${playerPos.top}px`, left: `${playerPos.left}px` }} id='character'>
                    ...
                    {/* Front feet */}
                    <div className='foot front left'></div>
                    <div className='foot front right'></div>

                    {/* Back feet */}
                    <div className='foot back left'></div>
                    <div className='foot back right'></div>
                </Character>
            </MazeCont>
            {displayGameWon ? (
                <GameWon>
